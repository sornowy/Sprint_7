...
from test_data import create_courier
...
class TestCourierLogin:
    ...
    def test_courier_login_success(self, create_courier_fixture):
        login_pass = create_courier_fixture
        login = login_pass[0]
        password = login_pass[1]
        response = requests.post(variables.COURIER_LOGIN_URL, json={"login": login, "password": password})
    ...
    def test_courier_login_miss(self, create_courier_fixture):
        login = create_courier_fixture[0]
        payload = {
            "login": login
        }
        response = requests.post(variables.COURIER_LOGIN_URL, json=payload)
    ...
    def test_courier_password_miss(self, create_courier_fixture):
        password = create_courier_fixture[1]
        payload = {
            "password": password
        }
        response = requests.post(variables.COURIER_LOGIN_URL, json=payload)
    ...
    def test_courier_invalid_filed(self, create_courier_fixture):
        login_pass = create_courier_fixture
        login = login_pass[0]
    ...
    def test_courier_authorized_duplicate(self, create_courier_fixture):
        login_pass = create_courier_fixture
